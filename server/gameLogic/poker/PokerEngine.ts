...
// Типы для совместимости со старым клиентом
interface OldCard {
  rank: string;
  suit: 'H' | 'D' | 'C' | 'S';
}

interface OldPokerPlayer {
  id: string;
  name: string;
  stack: number;
  hand: OldCard[];
  cards: OldCard[]; // Для совместимости
  currentBet: number;
  inHand: boolean;
  hasActed: boolean;
...
interface OldPokerGameState {
  stage: 'pre-flop' | 'flop' | 'turn' | 'river' | 'showdown';
  pot: number;
  communityCards: OldCard[];
  players: OldPokerPlayer[];
  status: string; // 'waiting' | 'in_progress' | 'finished'
  currentPlayerId: string | null; // ID игрока, чей ход
  ...
  // Поля для подсветки победных карт
  winningHandCards?: OldCard[];
}
...
export class PokerEngine {
  ...
  private handWinners: HandResult[] = []; // Сохраняем информацию о победителях раздачи
  
  // Callback функции для событий
  public onStateChange?: (state: GameState) => void;
  public onPlayerEvent?: (playerId: string, event: PokerEventData) => void;
  public onGameEvent?: (event: PokerEventData) => void;

  /**
   * Конвертирует карту из внутреннего формата в формат старого клиента
   */
  private convertCardToOldFormat(card: PokerCard): OldCard {
    const suitMap: { [key: string]: 'H' | 'D' | 'C' | 'S' } = {
      'hearts': 'H',
      'diamonds': 'D', 
      'clubs': 'C',
      'spades': 'S'
    ...
    return {
      rank: card.rank,
      suit: suitMap[card.suit] || card.suit as ('H' | 'D' | 'C' | 'S')
    };
  }
   ...
   */
  private convertCardsToOldFormat(cards: PokerCard[]): OldCard[] {
   ...
   */
  private convertPlayerToOldFormat(player: PokerPlayer, showCards: boolean = false): OldPokerPlayer {
  ...
  constructor(gameId: string, public settings: PokerSettings) {
    ...
    this.gameFlow = new GameFlow(settings);
  ...
  public playerAction(
    playerId: string, 
    actionData: { action: PokerAction; amount?: number }
  ): boolean {
   ...
   * Начинаем новый раунд торгов
   */
  private startNewBettingRound(): void {
  ...
  private completeHand(): void {
    ...
    const winners = HandEvaluator.determineWinners(
      this._players, 
      this.gameFlow.getCommunityCards()
    );
    ...
    this.emitGameEvent({
      ...
      data: { 
        winners: winners.map(w => ({
          ...
          amount: distributions.reduce((sum, d) => 
            sum + (d.winners.find(dw => dw.playerId === w.player.id)?.amount || 0), 0
          ),
   ...
   */
  private canStartNewHand(): boolean {
    const activePlayers = this._players.filter(p => 
      p.stack > 0 && p.hasBoughtIn
    );
   ...
   */
  public getPlayer(playerId: string): PokerPlayer | null {
   ...
   */
  public getPlayerGameState(playerId: string): OldPokerGameState | null {
    ...
    const baseState: OldPokerGameState = {
      stage: gameState.stage as any,
      pot: gameState.pot,
   ...
   */
  private getValidActions(player: PokerPlayer): PokerAction[] {
   ...
   */
  private emitGameEvent(event: PokerEventData): void {
    if (this.onGameEvent) {
   ...
   */
  public makeMove(playerId: string, move: any): any {
    ...
    const actionData = {
      action: move.action as PokerAction,
      amount: move.amount
    };
   ...
   */
  public isValidMove(playerId: string, move: any): boolean {
    const player = this.getPlayer(playerId);
   ...
   */
  public playerRebuy(playerId: string, amount: number): { success: boolean; error?: string } {
    const player = this.getPlayer(playerId);
    if (!player) {
   ...
   */
  public playerCashOut(playerId: string): { success: boolean; cashOutAmount?: number; error?: string } {
    const player = this.getPlayer(playerId);
    if (!player) {
      return { success: false, error: 'Player not found' };
    }
