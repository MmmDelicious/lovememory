import { PokerPlayer, SidePot, PotDistribution, HandResult } from '../../types/poker.types';
...
export class PotManager {
   ...
   */
  public calculatePots(players: PokerPlayer[]): void {
    ...
    for (let i = 0; i < bettingPlayers.length; i++) {
      ...
      remainingPlayers = remainingPlayers.filter(p => 
        p.totalBetThisHand > currentLevel || p.status === 'all-in'
      );
    }
  }
   ...
   */
  public distributeWinnings(winners: HandResult[]): PotDistribution[] {
   ...
   */
  public returnUncalledBets(players: PokerPlayer[]): void {
    ...
    if (playersNotAllIn.length === 0) return;
   ...
   */
  public getPotInfo(): { main: number; sides: SidePot[] } {
    return {
      main: this.mainPot,
      sides: [...this.sidePots]
    };
