...
class GameFlow {
     ...
     */
    findNextActiveSeat(players, startSeat) {
        const activePlayers = players.filter(p => p.status !== 'busted');
     ...
     */
    resetPlayersForNewHand(players) {
    ...
    dealHoleCards(players) {
        ...
        const activePlayers = players.filter(p => p.hasBoughtIn && p.stack > 0 && p.status !== 'busted');
    ...
    determineNextPlayer(players) {
        ...
        const activePlayers = players.filter(p => p.status === 'playing' && !p.hasActed);
     ...
     */
    determineNextShowdownPlayer(players) {
        // Игроки, которые могут принять решение в showdown
        const showdownPlayers = players.filter(p => (p.status === 'playing' || p.status === 'all-in') &&
            !p.hasActed &&
    ...
    isBettingRoundComplete(players) {
        ...
        // Обычная логика для торгов
        const activePlayers = players.filter(p => p.status === 'playing' || p.status === 'all-in');
        if (activePlayers.length <= 1) {
            return true;
        }
        // Все активные игроки должны были действовать
        const playersNeedingAction = activePlayers.filter(p => !p.hasActed && p.status === 'playing');
