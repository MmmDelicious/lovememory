...
exports.WordleGameNew = void 0;
const RealtimeGame_1 = require("../base/RealtimeGame");
 ...
 */
class WordleGameNew extends RealtimeGame_1.RealtimeGame {
    ...
    _executeMove(playerId, move) {
        ...
        // Оцениваем догадку
        const result = evaluateGuess(normalizedGuess, this._targetWord);
    ...
    _checkRoundCompletion() {
        const allPlayersFinished = this._players.every(player => this._playersFinished[player.id] === true);
...
_getPlayerTeam(playerId, string);
string | null;
{
    if (this._gameFormat !== '2v2')
        return null;
...
boolean;
{
    try {
        ...
        return dictionary.some((dictWord) => dictWord.toLowerCase() === normalizedWord && dictWord.length === word.length);
    }
    catch (error) {
...
}
{
    ...
    const wordsGuessed = (this._playerResults[playerId] || []).filter((result) => result.every((r) => r === 'correct')).length;
...
hasPlayerFinishedRound(playerId, string);
boolean;
{
...
//# sourceMappingURL=WordleGameNew.js.map